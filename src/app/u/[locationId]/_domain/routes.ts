...
export function findRoute(pathname: string, routesList: AdminRoute[] = routes): AdminRoute | undefined {
        ...
        if (activeRoute) {
            ...
            if (i > 0) {
                ...
                if (parentRoute?.childRoutes) {
                    siblingRoutes = parentRoute.childRoutes.filter((route) => route.path !== activeRoute.path);
                }
            } else {
