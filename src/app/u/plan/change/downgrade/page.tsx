...
import { type StripeCustomerId, type StripeRefundId, type StripeSubscriptionId } from '~/app/_domain/stripe';
...
import { getActiveStripeSubscriptionItem, getActiveSubscriptionItemId } from '~/app/_utils/stripe-utils';
import { env } from '~/env';
...
async function Step2StripeProcessing(props: { fromTier: PriceTier; toTier: PriceTier; orgId: string }) {
    ...
    const stripeCustomerId = (await getCustomerByOrgId(orgId)).stripeCustomerId as StripeCustomerId;
    if (!stripeCustomerId) {
        throw new AppError({
