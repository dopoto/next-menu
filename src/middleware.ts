  ...
  async (auth, req: NextRequest) => {
    ...
    if (userId && isMyRoute(req)) {
      if (!currentLocationId || !orgId) {
        console.log(
          `DBG-MIDDLEWARE [/my] Not onboarded, redirecting to /sign-up/onboarding-incomplete. currentLocationId: ${currentLocationId}, orgId: ${orgId}`,
        );
        const signUpUrl = new URL("/sign-up/onboarding-incomplete", req.url);
